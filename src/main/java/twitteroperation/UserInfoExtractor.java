...
public class UserInfoExtractor
{
     ...
     */
    public void getUsersFollowing(Dataset dataset, Dataset clusterizedDataset, TwitterFactory twitterFactory) throws TwitterException {
        try{
            ...
            for (Map.Entry<Integer, UserModel> entry: users.entrySet()) {
                ...
                for (long i : ids.getIDs()) {
                    if (clusterizedDataset.exixstObj(i+"")) {
                        UserModel u = twitterFactory.getInstance(i+"");
                        dataset.addUser(u);
                    }
                }
    ...
    public static void main(String[] args) throws Utils.CacheNotPresent, TwitterException {
        ...
        userInfoExtractor.getUsersFollowing(d22);
